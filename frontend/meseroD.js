import { API_AUTH_LOGIN, API_BASED, API_REPORTES, API_URL } from "./config.js";
...
try {
  ...
  // 🔹 Enviar a ambos endpoints al mismo tiempo
  ...
  if (resp1.ok && resp2.ok) {
    ...
    const data = await resp1.json(); // usamos la respuesta del primero

    console.log("Orden creada con éxito en ambos endpoints:", data);

    mostrarNotificacion("Orden enviada correctamente", "#27ae60");
    ...
    setTimeout(async () => {
      try {
        ...
        console.log("Ya se imprimió o no hay worker"); 
      }
    }, 800);
    ...
    console.error("Error servidor API_BASE:", error1);
    console.error("Error servidor API_BASED:", error2);

    mostrarNotificacion("Error al enviar orden a uno de los endpoints", "#e74c3c");
  }
